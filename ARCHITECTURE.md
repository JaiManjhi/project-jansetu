...
| Voice input | **As built:** record with `MediaRecorder`, transcribe server-side with `whisper-large-v3` on Groq via `POST /api/transcribe`. Web Speech API was the original choice and has been removed | Web Speech API needed zero backend and zero key, so it shipped first — but it does not work on the phones citizens report from. Android Chrome mishandles `continuous` and ends the session mid-sentence; iOS Safari produces nothing unless Siri dictation is enabled. Both fail the same way: the mic light comes on, the citizen speaks, nothing appears. Recording and transcribing server-side behaves identically everywhere, costs ~1.7s round trip, and is markedly more accurate on Indian-accented Hindi and English. Model revised from `-turbo` to full `large-v3` on 2026-09-05 after a user reported poor Bengali and Marathi accuracy — 10.3% WER vs 12%, for ~60ms — and capture was moved to 16 kHz mono, which is what Groq downsamples to anyway and is a far smaller upload on a rural connection |
...
│   ├── sw.js                         # service worker (hand-rolled, not next-pwa)
│   ├── icon-192.png / icon-512.png   # generated by scripts/generate-icons.mjs
│   └── maplibre-gl-worker.mjs        # copied by scripts/sync-map-worker.mjs
├── data/
