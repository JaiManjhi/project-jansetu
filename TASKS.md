...
- [x] Photo upload via Cloudinary — signed direct-to-Cloudinary upload via `POST /api/media/sign`. **Blocked on credentials:** the Cloudinary admin API rejects the current key/secret against cloud name `jansetu` with `cloud_name mismatch`, so a real upload is unverified. Signing route and UI are done and degrade cleanly.
